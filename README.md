...
AI를 잘 모르는 분에게 가장 먼저 설명해야 하는 것은 기술 이름이 아니라 문제입니다.
...
이 프로젝트는 단순 문서 검색기보다 한 단계 더 나아가 있습니다.  
그 이유는 실제 사용자가 질문을 한 번만 하지 않기 때문입니다.

예를 들어 이런 식으로 묻는다고 생각해보면 됩니다.
...
여기서 가장 중요한 설계는 **검색용 정보와 생성용 정보를 분리했다는 것**입니다.

`retrieval_query`에는 검색에 필요한 정보만 넣습니다.  
반면 `generation_hints`에는 답변 스타일과 개인화 힌트만 넣습니다.

이렇게 한 이유는 명확합니다.

- 검색 질의에 스타일 정보를 넣으면 검색이 흐려질 수 있고
- 답변 생성 단계에는 개인화 정보가 필요하기 때문입니다.

즉 발표에서는 이렇게 말할 수 있습니다.

> 이 단계는 사용자의 질문을 그대로 던지는 것이 아니라, 검색용 질문과 답변용 힌트를 분리해서 준비하는 단계입니다.
